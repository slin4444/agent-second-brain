        ...
        with urllib.request.urlopen(req, timeout=120) as response:
            ...
            for action in res.get("actions", []):
                ...
                if a_type in ["create", "update_or_create"]:
                    ...
                    # Блок-лист: не создавать файлы с ошибками скрейпинга
                    BLOCKLIST = ["408", "scrape_http", "http_error"]
                    if any(b in p.stem.lower() for b in BLOCKLIST):
                        logger.info(f"BLOCKED: {p.name} (blocklist match)")
                        continue
