...
def build_sources_block_html(note_files):
    """HTML-блок со ссылками на исходные заметки для Telegram."""
    if not note_files:
        return ""
    lines = ["\n\n<b>📚 Источники:</b>"]
    for f in note_files:
        lines.append(f"• <code>{f.stem}</code>")
    return "\n".join(lines)

def build_sources_block_md(note_files):
    """Markdown-блок со ссылками на исходные заметки для Obsidian."""
    if not note_files:
        return ""
    lines = ["\n\n## 📚 Источники"]
    for f in note_files:
        # Вычисляем относительный путь от корня vault для wikilink
        try:
            rel = f.relative_to(VAULT_DIR)
            lines.append(f"- [[{rel.with_suffix('')}|{f.stem}]]")
        except ValueError:
...
- Ссылайся на исходные заметки по имени в формате [[имя_файла]] — вставляй их прямо в текст рядом с упоминанием идеи.
- Добавь в конце блок "🎯 ACTION ITEMS" (что нужно сделать на основе этих знаний).
- НЕ добавляй блок источников в конце — он будет добавлен автоматически.
...
""" + "\n\n".join(notes)
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=60) as response:
            ...
            sources_html = build_sources_block_html(note_files)
            sources_md   = build_sources_block_md(note_files)

            # Сохранение в Obsidian (Markdown + wikilinks)
            DAILY_DIR.mkdir(parents=True, exist_ok=True)
            md_content = res + sources_md
            (DAILY_DIR / f"Summary_{datetime.now().strftime('%Y-%m-%d')}.md").write_text(md_content, encoding="utf-8")
            (DAILY_DIR / "Summary_Latest.md").write_text(md_content, encoding="utf-8")

            # Отправка в Telegram (HTML + code-блок с именами файлов)
            send_telegram_html(res + sources_html)
